...
class temperaturas: #definimos la clase
	promedio_centigrados = 0 #almacena en promedio °C
	prmedio_farentheit = 0 #almacena el pomedio de °F
	...
	def datosTemperaturas(self): #metodo llevara el recabar los datos
		fecha = input("Inserta la fecha: ") #pide la fecha
		self.fechas.append(fecha) #alamacena la fecha en la variable fechas
		temperatura = int(input("Inserte la temperatura en °C: "))  #pide la temperatura en °c y de tipo entero
		self.centigrados.append(temperatura) #almacena en la variable centigrados
		convertir = (temperatura * 9 / 5) + 32 #convierte los °c a °f
		self.farentheit.append(convertir) #almacena en la variable farentheit

	def convertiryBuscar(self): #metodo convertir y buscar
		datos = dict(zip(self.farentheit,self.fechas))  #convierte fechas y farentheit a diccionario
		maximo = max(datos.items(), key=lambda x: x[1])  #busca el mayor dato posible, items()Devuelve una lista de tuplas, cada tupla se compone de dos elementos: el primero será la clave y el segundo, su valor.#Key retorna una lista de elementos, los cuales serán las claves de nuestro diccionario#lambada convierte una funcion a algo anonimo
		abrir = open("Temperaturas.txt", "a")  #abre el archivo en modo append o agregar en español
		...
		print("Temperatura mas alta es de: " + str(maximo)) #imprime la temperatura mas alta y su fecha
	...
	def archivo(self):  #metodo archivo
		abrir = open("Temperaturas.txt", "a")  #abre el archivo en modo append o agregar
...
while respuesta == "S" or respuesta == "s": #mientras respuesta sea S o S
	...
	objeto.datosTemperaturas()  #llama al metodo de los datos
	respuesta = input("¿Desea leer otra fecha y temperatura? S/N ")  #pregunta si quieres leer otra fecha y temperatura
